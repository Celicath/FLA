...
void draw_small_num(int size, int x, int y, int num, int space, int fill_color, int back_color)
{
	...
	if (num < 0)
	{
		for (int i = 0; i < 3 * size; i++)
			for (int j = 0; j < size; i++)
				VRAM[pos + i + (4 + j) * LCD_WIDTH_PX] = (color_t)fill_color;
		pos += size * 4;
		num = -num;
	}
	do
		...
		for (int i = 0; i < size; i++)
			for (int j = 0; j < size; j++)
			{
				VRAM[pos + i + j * LCD_WIDTH_PX] =
					(color_t)(k != 1 ? fill_color : back_color);
				VRAM[pos + i + (j + size) * LCD_WIDTH_PX] =
					(color_t)((k == 0 || k >= 4) ? fill_color : back_color);
				VRAM[pos + i + (j + 2 * size) * LCD_WIDTH_PX] =
					VRAM[pos];
				VRAM[pos + i + (j + 3 * size) * LCD_WIDTH_PX] =
					(color_t)(((k & 1) == 0 && k != 4) ? fill_color : back_color);
				VRAM[pos + i + (j + 4 * size) * LCD_WIDTH_PX] =
					(color_t)((k % 3 != 1) ? fill_color : back_color);

				VRAM[pos + i + size + j * LCD_WIDTH_PX] =
					(color_t)((k != 1 && k != 4) ? fill_color : back_color);
				VRAM[pos + i + size + (j + 2 * size) * LCD_WIDTH_PX] =
					(color_t)((k >= 2 && k != 7) ? fill_color : back_color);
...
void border_helper(int x, int y, int dx, int dy, int color)
{
	color_t* VRAM = (color_t*)GetVRAMAddress();

	for (int i = 0; i < 20; i++)
	{
		VRAM[y * LCD_WIDTH_PX + x] = (color_t)color;
		if (i < 5) x += dx;
		else if (i < 7) y += dy;
		else if (i < 10) x -= dx;
		else if (i < 13) y += dy;
